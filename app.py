...
def get_db_connection():
    if not DATABASE_URL:
        raise RuntimeError("DATABASE_URL is missing in Railway Variables.")
...
def init_db():
    try:
        ...
        columns = {
            ...
            "image_url": "TEXT", "source_url": "TEXT", "source_name": "TEXT",
            "original_title": "TEXT", "seo_title": "TEXT", "meta_description": "TEXT",
            "slug": "TEXT", "primary_keyword": "TEXT", "secondary_keywords": "TEXT",
            "search_intent": "TEXT", "trend_score": "INTEGER DEFAULT 0", "seo_score": "INTEGER DEFAULT 0",
            "quality_score": "INTEGER DEFAULT 0", "seo_reason": "TEXT", "faq_json": "TEXT", "schema_json": "TEXT"
        }

        for column, column_type in columns.items():
            cur.execute(f"ALTER TABLE articles ADD COLUMN IF NOT EXISTS {column} {column_type};")
...
def init_jobs_db():
    try:
        ...
        columns = {
            "category": "TEXT", "source_site": "TEXT", "source_url": "TEXT",
            "title_ar": "TEXT", "company_ar": "TEXT", "description_ar": "TEXT",
            "conditions_ar": "TEXT", "documents_ar": "TEXT", "how_to_apply_ar": "TEXT",
            "deadline": "TEXT", "offer_hash": "TEXT",
        }

        for column, column_type in columns.items():
            cur.execute(f"ALTER TABLE job_offers ADD COLUMN IF NOT EXISTS {column} {column_type};")
...
def generate_with_groq(prompt):
    ...
    for attempt in range(total_keys):
        ...
        try:
            ...
            if not result: raise Exception("Empty Groq response")
    ...
    try:
        ...
        cur.execute(
            ...
            INSERT INTO job_offers (
                category, source_site, source_url, title_ar, company_ar,
                description_ar, conditions_ar, documents_ar, how_to_apply_ar,
                deadline, offer_hash, created_at
            ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, CURRENT_TIMESTAMP)
            ON CONFLICT (offer_hash) DO NOTHING
            """,
            (
                category, offer.get("source_site"), offer.get("source_url"),
                offer.get("title_ar"), offer.get("company_ar"), offer.get("description_ar"),
                offer.get("conditions_ar"), offer.get("documents_ar"), offer.get("how_to_apply_ar"),
                offer.get("deadline"), offer.get("offer_hash"),
            )
        )
        conn.commit()
...
def process_job_category(category):
    ...
    for site_name, site_url in info["sites"]:
        ...
        for offer in offers:
            ...
            if not offer_hash or job_offer_exists(offer_hash): continue
...
# ============================================================
# IMAGE / COUNTRY / LANG / RSS
# ============================================================

def generate_image(prompt):
    if not prompt: prompt = "international news"
...
def detect_country():
    ...
    if saved_country in COUNTRIES: return saved_country
    try:
        ip = get_client_ip()
        try:
            ip_obj = ipaddress.ip_address(ip)
            if ip_obj.is_private or ip_obj.is_loopback or ip_obj.is_reserved: return "ma"
        except: pass
        response = requests.get(f"[https://ipapi.co/](https://ipapi.co/){ip}/json/", timeout=3)
        if response.ok:
            data = response.json()
            country_code = data.get("country_code", "").lower()
            if country_code in COUNTRIES: return country_code
    except Exception as e:
...
def detect_language(country):
    ...
    if saved_language in LANGUAGES: return saved_language
    accept_language = request.headers.get("Accept-Language", "").lower()
    for language in LANGUAGES:
        if accept_language.startswith(language): return language
...
def get_country_news(country):
    if country not in COUNTRIES: country = "ma"
    google_country = COUNTRIES[country]["google_news"]
    url = f"[https://news.google.com/rss?hl=en-US&gl=](https://news.google.com/rss?hl=en-US&gl=){google_country}&ceid={google_country}:en"
    try:
...
def get_google_trends(country):
    ...
    try:
        feed = feedparser.parse(url)
        return [entry.get("title", "").strip() for entry in feed.entries[:TRENDING_LIMIT] if entry.get("title")]
    except Exception as e:
...
def generate_with_groq_compound(prompt, max_tokens=5000):
    global current_groq_key
    if not NEWS_GROQ_KEYS: return None
    ...
    for _ in range(total):
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are a web research and SEO intelligence agent. Return valid JSON."},
                    {"role": "user", "content": prompt},
                ],
...
def analyze_trend_and_seo(news_item, country, trend_terms):
    ...
    prompt = f"""Analyze this news opportunity for {country_name}... [TRUNCATED FOR BREVITY - keep logic exactly similar] Return JSON."""
    ...
    if not data:
        ...
        data = {
            "publish": True, "reason": "Fallback", "trend_score": score,
            "seo_score_before_writing": 55, "primary_keyword": title[:80],
            "secondary_keywords": [], "search_intent": "news",
            "content_angle": "Explain event.", "competitor_gaps": [],
            "recommended_title": title, "meta_description": title[:155],
            "slug": slug, "source_quality": 50, "evidence_notes": "No web research."
        }
    data["trend_score"] = int(max(0, min(100, data.get("trend_score", 0))))
    return data

def research_story(news_item):
    prompt = f"Verify this news item... Return ONLY JSON."
    raw = generate_with_groq_compound(prompt, max_tokens=3000) if SEO_RESEARCH_ENABLED else None
    data = _clean_json(raw)
    if not data:
        return {
            "verified": bool(news_item.get("link")), "confidence": 50 if news_item.get("link") else 20,
            "confirmed_facts": [news_item.get("description", "")], "uncertain_claims": [],
            "source_urls": [news_item.get("link", "")], "source_names": [news_item.get("source", "")],
        }
    return data
...
def generate_article(news_item, country, seo_plan=None, verification=None):
    seo_plan, verification = seo_plan or {}, verification or {}
    country_name = COUNTRIES.get(country, COUNTRIES["ma"])["name"]
    prompt = f"Create a high-quality, original news article based ONLY on the verified material below... Return ONLY JSON."
...
def quality_check(article_data, news_item, verification, seo_plan):
    prompt = f"Audit this proposed news article for publication... Return JSON."
    raw = generate_with_groq_compound(prompt, max_tokens=3000) if SEO_RESEARCH_ENABLED else None
    data = _clean_json(raw)
    if not data:
        local = seo_score_local(article_data)
        return {
            "publish": local >= SEO_MIN_SCORE, "factual_accuracy": 70, "originality": 70,
            "usefulness": 70, "readability": 75, "seo": local, "source_quality": int(seo_plan.get("source_quality", 50)),
            "overall": local, "issues": [], "fixes": [],
        }
...
def optimize_article(article_data, audit, seo_plan, news_item, verification):
    if audit.get("overall", 0) >= max(SEO_MIN_SCORE, QUALITY_MIN_SCORE): return article_data
    prompt = f"Improve this news article using the audit. Keep every factual claim supported... Return JSON."
    raw = generate_with_groq(prompt)
    data = _clean_json(raw)
    if not data: return article_data
...
def build_schema(article_id, country, lang, article, faq):
    ...
    graph = [{
        "@type": "NewsArticle", "@id": url + "#article", "mainEntityOfPage": {"@type": "WebPage", "@id": url},
        ...
        "image": [article.get("image", "")], "datePublished": str(article.get("created_at", "")),
    ...
    if faq:
        graph.append({
            "@type": "FAQPage",
            "mainEntity": [
                {"@type": "Question", "name": x.get("question", ""), "acceptedAnswer": {"@type": "Answer", "text": x.get("answer", "")}}
...
def build_website_schema():
    ...
    return {
        ...
        "@graph": [
            {"@type": "Organization", "@id": home_url + "#organization", "name": "Corvex News", "url": home_url},
            {"@type": "WebSite", "@id": home_url + "#website", "url": home_url, "name": "Corvex News", "publisher": {"@id": home_url + "#organization"}},
        ],
    }
...
def looks_like_lang(text, lang):
    ...
    if lang == "ar": return arabic_ratio > 0.4
...
def safe_translate(text, target_lang):
    ...
    try:
        ...
        return " ".join(t for t in [GoogleTranslator(source="auto", target=target_lang).translate(chunk) for chunk in chunks] if t)
    except Exception as e:
        print(f"!! deep-translator fallback failed ({target_lang}): {e}")
        return None

def cache_translation(article_id, lang, title, content):
    if lang not in LANGUAGES or lang == "en": return
    try:
        conn = get_db_connection(); cur = conn.cursor()
        cur.execute(f"UPDATE articles SET title_{lang} = %s, content_{lang} = %s WHERE id = %s", (title, content, article_id))
        conn.commit()
        cur.close(); conn.close()
    except Exception as e:
...
def translate_article(title, content, language):
    language_names = {"ar": "Arabic", "fr": "French", "en": "English", "es": "Spanish"}
    target = language_names.get(language, "English")
    prompt = f"Translate this news into {target} preserving facts. Return ONLY valid JSON: {{\"title\": \"...\", \"content\": \"...\"}}\n\nTITLE:\n{title}\n\nCONTENT:\n{content}"
    raw = generate_with_groq(prompt)
    data = _clean_json(raw)
    ...
    if fallback_title or fallback_content:
        return {"title": fallback_title or title, "content": fallback_content or content}
    return None

# ============================================================
# CHECK DUPLICATE & SAVE ARTICLE
# ============================================================

def article_exists(country, source_url):
    try:
        conn = get_db_connection(); cur = conn.cursor()
        cur.execute("SELECT id FROM articles WHERE country = %s AND source_url = %s LIMIT 1", (country, source_url))
        result = cur.fetchone()
        cur.close(); conn.close()
        return result is not None
    except: return False
...
def save_article(country, news_item, article_data, translations, audit=None, seo_plan=None):
    try:
        ...
        title_en = article_data.get("title", news_item.get("title", "News"))
        content_en = article_data.get("content", "")

        def resolve_lang(lang_dict, lang_code):
            t = lang_dict.get("title")
            c = lang_dict.get("content")
            if not t: t = safe_translate(title_en, lang_code) or title_en
            if not c: c = safe_translate(content_en, lang_code) or content_en
            return t, c

        title_ar, content_ar = resolve_lang(translations.get("ar", {}), "ar")
        title_fr, content_fr = resolve_lang(translations.get("fr", {}), "fr")
        title_es, content_es = resolve_lang(translations.get("es", {}), "es")
        image_url = generate_image(article_data.get("image_prompt", news_item.get("title", "")))

        seo_plan, audit = seo_plan or {}, audit or {}
        ...
        cur.execute("""
            INSERT INTO articles (
                country, region, category, title_ar, title_fr, title_en, title_es,
                content_ar, content_fr, content_en, content_es, image_url, source_url, source_name, original_title,
                seo_title, meta_description, slug, primary_keyword, secondary_keywords, search_intent, trend_score, seo_score,
        ...
        """, (
            country, region, category, title_ar, title_fr, title_en, title_es,
            content_ar, content_fr, content_en, content_es, image_url, news_item.get("link", ""), news_item.get("source", ""), news_item.get("title", ""),
            seo_title, meta_description, slug, primary_keyword, secondary_keywords, article_data.get("search_intent") or "news",
            int(seo_plan.get("trend_score", 0)), int(audit.get("seo", seo_score_local(article_data))), int(audit.get("overall", 0)),
            json.dumps({"reason": seo_plan.get("reason", "")}, ensure_ascii=False), json.dumps(faq, ensure_ascii=False),
            json.dumps(build_schema(0, country, "en", {"title": title_en, "content": content_en}, faq), ensure_ascii=False)
        ))
        conn.commit()
        inserted = cur.rowcount > 0
        cur.close(); conn.close()
...
def process_country(country):
    print(f"\n====================================\n-> AI SEO Robot checking country: {country}\n====================================")
    robot_status["current_country"] = country
    news = get_country_news(country)
    trends = get_google_trends(country)
    saved = 0
    for news_item in news:
        if saved >= ARTICLES_PER_COUNTRY: break
        if news_item.get("link", "") and article_exists(country, news_item["link"]): continue
        seo_plan = analyze_trend_and_seo(news_item, country, trends)
        if not seo_plan.get("publish", True): continue
        verification = research_story(news_item)
        if not verification.get("verified", False): continue
        ...
        if int(audit.get("overall", 0)) < QUALITY_MIN_SCORE: article_data = optimize_article(article_data, audit, seo_plan, news_item, verification)
        translations = {lang: translate_article(article_data.get("title", news_item.get("title", "")), article_data.get("content", ""), lang) for lang in ("ar", "fr", "es")}
        if save_article(country, news_item, article_data, {k: v for k, v in translations.items() if v}, audit, seo_plan):
            saved += 1
...
def run_robot():
    if not robot_lock.acquire(blocking=False): return
    robot_status.update({"running": True, "current_country": None, "last_run_start": datetime.now(), "total_articles_this_run": 0})
    try:
        for country in COUNTRIES.keys():
            try: process_country(country)
            except Exception as e: print(f"!! Country {country} failed: {e}")
            time.sleep(3)
    finally:
        robot_status.update({"running": False, "current_country": None, "last_run_end": datetime.now(), "last_run_saved": robot_status["total_articles_this_run"]})
        robot_lock.release()
...
# ============================================================
BASE_CSS = """
:root { --bg: #0b0e14; --surface: #12161f; --surface-2: #1a1f2b; --border: #232938; --text: #eef1f6; --text-dim: #9aa3b5; --accent: #4f7cff; --accent-2: #22c55e; }
...
header.site-header { position: sticky; top: 0; z-index: 10; background: rgba(11,14,20,0.92); backdrop-filter: blur(8px); border-bottom: 1px solid var(--border); padding: 14px 20px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px; }
.logo { font-weight: 800; font-size: 1.3rem; letter-spacing: 0.5px; color: var(--accent); }
.logo a { color: inherit; }
.controls { display: flex; gap: 10px; flex-wrap: wrap; align-items: center; }
.controls select { background: var(--surface-2); color: var(--text); border: 1px solid var(--border); border-radius: 8px; padding: 8px 10px; font-size: 0.9rem; }
...
.card { background: var(--surface); border: 1px solid var(--border); border-radius: 14px; overflow: hidden; display: flex; flex-direction: column; transition: transform 0.15s ease, border-color 0.15s ease; }
...
.category { align-self: flex-start; background: var(--surface-2); color: var(--accent); font-size: 0.72rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; padding: 3px 9px; border-radius: 999px; }
...
.no-news { text-align: center; padding: 60px 20px; color: var(--text-dim); }
...
.content { white-space: pre-line; font-size: 1.05rem; max-width: 760px; margin: 0 auto; }
.source, .faq { margin-top: 30px; padding-top: 20px; border-top: 1px solid var(--border); max-width: 760px; margin: 30px auto 0; }
.faq h2 { font-size: 1.35rem; }
.faq h3 { margin: 18px 0 5px; font-size: 1rem; }
.faq p { color: var(--text-dim); margin: 0; }
footer { text-align: center; color: var(--text-dim); font-size: 0.8rem; padding: 30px 20px; border-top: 1px solid var(--border); margin-top: 30px; }
@media (max-width: 768px) { header.site-header { padding: 12px 14px; } .controls { width: 100%; justify-content: space-between; } .grid { grid-template-columns: 1fr; gap: 14px; } }
"""
...
<header class="site-header">
    <div class="logo notranslate" translate="no"><a href="/">CORVEX NEWS</a></div>
    <div class="controls">
        <a href="{{ url_for('jobs_page') }}" style="align-self:center; color:var(--accent); font-weight:700; font-size:0.9rem;">الخدمة / Jobs</a>
        <form method="get"><input type="hidden" name="country" value="{{ current_country }}"><select name="lang" onchange="this.form.submit()">{% for code, name in languages.items() %}<option value="{{ code }}" {% if code == current_language %}selected{% endif %}>{{ name }}</option>{% endfor %}</select></form>
        <form method="get"><input type="hidden" name="lang" value="{{ current_language }}"><select name="country" onchange="this.form.submit()">{% for code, info in countries.items() %}<option value="{{ code }}" {% if code == current_country %}selected{% endif %}>{{ info.native }}</option>{% endfor %}</select></form>
    </div>
</header>
<main>
    <div class="hero">
    ...
    {% if articles %}
        ...
        {% for article in articles %}
            <article class="card">
                {% if article.image %}<img src="{{ article.image }}" alt="{{ article.title }}" loading="lazy">{% endif %}
                <div class="card-content">
                    <span class="category">{{ article.category }}</span>
                    <h2>{{ article.title }}</h2>
                    <p>{{ seo_description(article.content) }}</p>
                    <a href="/article/{{ article.id }}?country={{ current_country }}&lang={{ current_language }}" class="read">Read more &rarr;</a>
                </div>
            </article>
        {% endfor %}
        </div>
    {% else %}
        <div class="no-news"><h2>No news found.</h2><p>Try another country or language.</p></div>
    {% endif %}
</main>
<footer>
...
<main>
    <div class="article-header">
        <a href="/?country={{ current_country }}&lang={{ current_language }}" class="back">&larr; Back to News</a><br>
        <span class="category">{{ article.category }}</span>
        <h1>{{ article.title }}</h1>
    ...
    <div class="source">
        Source: <a href="{{ article.source_url }}" target="_blank" rel="noopener noreferrer">{{ article.source_name or 'Original Article' }}</a>
    </div>
    {% endif %}
</main>
<footer>
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>عروض العمل - Corvex News</title>
<style>{{ base_css }}</style>
</head>
<body>
<header class="site-header">
    <div class="logo notranslate" translate="no"><a href="/">CORVEX NEWS</a></div>
    <div class="controls">
        <a href="/" style="align-self:center; color:var(--accent); font-weight:700; font-size:0.9rem;">الأخبار / News</a>
        <form method="get">
            <select name="category" onchange="this.form.submit()">
                <option value="">جميع الفئات</option>
                {% for key, info in job_categories.items() %}
                <option value="{{ key }}" {% if key == current_category %}selected{% endif %}>{{ info.label }}</option>
                {% endfor %}
            </select>
        </form>
    </div>
...
<main>
    <div class="hero">
        <h1>آخر عروض العمل</h1>
        <p>عروض جديدة يتم تحديثها تلقائيا.</p>
    </div>
    {% if offers %}
        <div class="grid">
        {% for offer in offers %}
            <article class="card">
                <div class="card-content">
                    ...
                    <p><strong>الشركة:</strong> {{ offer.company_ar }}</p>
                    <p>{{ offer.description_ar }}</p>
                    <p><strong>الشروط:</strong> {{ offer.conditions_ar }}</p>
                    <p><strong>الوثائق:</strong> {{ offer.documents_ar }}</p>
                    <p><strong>طريقة الترشيح:</strong> {{ offer.how_to_apply_ar }}</p>
                    {% if offer.deadline and offer.deadline != 'غير محدد' %}<p><strong>آخر أجل:</strong> {{ offer.deadline }}</p>{% endif %}
                    {% if offer.source_url %}<a href="{{ offer.source_url }}" target="_blank" rel="noopener noreferrer" class="read">رابط العرض الأصلي &rarr;</a>{% endif %}
                </div>
            </article>
        {% endfor %}
        </div>
    {% else %}
        <div class="no-news"><h2>لا توجد عروض حاليا.</h2></div>
    {% endif %}
</main>
<footer>
    &copy; 2026 Corvex News.
...
def home():
    ...
    if country not in COUNTRIES: country = detect_country()
    lang = request.args.get("lang")
    if lang not in LANGUAGES: lang = detect_language(country)
    ...
    try:
        conn = get_db_connection(); cur = conn.cursor()
        cur.execute(f"SELECT id, title_{lang}, content_{lang}, title_en, content_en, image_url, category, source_name, created_at FROM articles WHERE country = %s ORDER BY created_at DESC LIMIT 30", (country,))
        rows = cur.fetchall()
        cur.close(); conn.close()

        for row in rows:
            article_id, title, content, fallback_title, fallback_content = row[0], row[1], row[2], row[3], row[4]
            if lang != "en" and (not looks_like_lang(title, lang) or not looks_like_lang(content, lang)):
                title = safe_translate(fallback_title if not looks_like_lang(title, lang) else title, lang) or fallback_title
                content = safe_translate(fallback_content if not looks_like_lang(content, lang) else content, lang) or fallback_content
                cache_translation(article_id, lang, title, content)
            articles.append({"id": article_id, "title": title, "content": content or "", "image": row[5], "category": row[6], "source": row[7], "created_at": row[8]})
    except Exception as e: print(f"!! Home database error: {e}")

    return render_template_string(HOME_TEMPLATE, base_css=BASE_CSS, articles=articles, countries=COUNTRIES, languages=LANGUAGES, current_country=country, current_language=lang, country_name=country_info["name"], seo_description=seo_description)
...
def article_detail(article_id):
    ...
    try:
        conn = get_db_connection(); cur = conn.cursor()
        cur.execute(f"SELECT id, title_{lang}, content_{lang}, title_en, content_en, image_url, category, source_url, source_name, original_title, created_at, country, seo_title, meta_description, slug, primary_keyword, secondary_keywords, search_intent, trend_score, seo_score, quality_score, seo_reason, faq_json, schema_json FROM articles WHERE id = %s LIMIT 1", (article_id,))
        row = cur.fetchone()
        cur.close(); conn.close()

        if row:
            title, content, fallback_title, fallback_content = row[1], row[2], row[3], row[4]
            if lang != "en" and (not looks_like_lang(title, lang) or not looks_like_lang(content, lang)):
                title = safe_translate(fallback_title if not looks_like_lang(title, lang) else title, lang) or fallback_title
                content = safe_translate(fallback_content if not looksأهلا! هاني راجعت الكود ديالك وصلحت فيه بعض الأخطاء والمشاكل اللي كانت باينة:

1. **إزالة `[source: 1]`**: حيدت ديك الكلمة من أول سطر حيت كادير خطأ برمجي (SyntaxError) وما كتخليش الكود يخدم.
2. **إصلاح `ARTICLE_TEMPLATE`**: زدت وسم الإغلاق `</footer>` اللي كان ناقص لتحت فـ HTML باش ما يوقعش مشكل فالواجهة ديال المقالات.

ها هو الكود كامل ومقاد:
...
JOB_SITES = {
    "private_ma": {
        "label": "الخدمة فالمغرب (القطاع الخاص)",
        "sites": [
            ("LinkedIn", "[https://www.linkedin.com/jobs/jobs-in-morocco/](https://www.linkedin.com/jobs/jobs-in-morocco/)"),
            ("ReKrute", "[https://www.rekrute.com/](https://www.rekrute.com/)"),
            ("Emploitic", "[https://emploitic.com/](https://emploitic.com/)"),
            ("Dreamjob", "[https://www.dreamjob.ma/](https://www.dreamjob.ma/)"),
            ("Careerlink", "[https://careerlink.ma/](https://careerlink.ma/)"),
        ],
    },
    "public_ma": {
        "label": "الوظائف العمومية (المغرب)",
        "sites": [
            ("Emploi Public", "[https://www.emploi-public.ma/](https://www.emploi-public.ma/)"),
            ("ANAPEC", "[https://www.anapec.org/](https://www.anapec.org/)"),
        ],
    },
    "gulf": {
        "label": "الخدمة فالخليج",
        "sites": [
            ("Bayt", "[https://www.bayt.com/](https://www.bayt.com/)"),
            ("Naukrigulf", "[https://www.naukrigulf.com/](https://www.naukrigulf.com/)"),
        ],
    },
    "abroad": {
        "label": "الخدمة فالخارج (أوروبا / كندا)",
        "sites": [
            ("ANAPEC Infitah", "[https://skills.ma/infitah/](https://skills.ma/infitah/)"),
            ("EURES", "[https://eures.europa.eu/](https://eures.europa.eu/)"),
            ("France Travail", "[https://candidat.francetravail.fr/](https://candidat.francetravail.fr/)"),
            ("Guichet-Emplois Canada", "[https://www.guichetemplois.gc.ca/](https://www.guichetemplois.gc.ca/)"),
        ],
    },
}
...
Hania, rani dert mraja3a l code li 3titini[cite: 1]. L-code mktoub mzyan b-Flask, walakin lqit fih chi aghlat sghar fl-HTML o chi t7sinat li khasshom ytdaro. 

Hado homa l-islahat li dert:
*   **Taqwim HTML (`ARTICLE_TEMPLATE`)**: nsiti matsddch tag dial `<footer>` f l-akhar dial template dial l-maqal, li kiged ydir mochkil fl-affichage f chi browsers[cite: 1].
*   **Tandif l-Code**: 7yydt `import hashlib` li m3awda west l-fonction `make_offer_hash` 7it deja kayna fl-foq m3a les imports lawlin[cite: 1].
