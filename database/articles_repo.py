...
from utils.seo_helpers import seo_description, build_schema
from utils.text_helpers import seo_score_local
...
def save_article(country, news_item, article_data, translations, safe_translate, audit=None, seo_plan=None):
    try:
        conn = get_db_connection()
        cur = conn.cursor()
...
def fetch_home_articles(country, lang):
...
def fetch_article_by_id(article_id, lang):
    title_column = f"title_{lang}"
    content_column = f"content_{lang}"
    try:
        conn = get_db_connection()
...
def fetch_seo_status():
    ...
    cur.close()
    conn.close()
    return total, avg_seo, avg_quality, avg_trend


def fetch_all_articles_for_translation_fix():
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute(
        "SELECT id, title_en, content_en, title_ar, content_ar, title_fr, content_fr, title_es, content_es FROM articles"
    )
    rows = cur.fetchall()
    cur.close()
    conn.close()
...
def update_translations(article_id, updates):
    conn = get_db_connection()
    cur = conn.cursor()
    set_parts = []
    params = []
